...
### Удаление ветки
Теперь ветка *master* актуальна. Ветка <сливаемая ветка> больше не нужна, и ее можно удалить. Для этого используем команду *git branch -d <имя ветки>*.
...
2. Открыть сам файл и отредактировать его вручную, записав туда что угодно. 
 ...
 В сообщении *merge-коммита* следует указывать полезную информацию: 
     ...
     1. Какие ветки вы слили;
     2. По какому принципу вы сливали файлы;
     3. Если были конфликты, то какие изменения   внесли и т.д. 
...
Команда для отправки изменений на сервер такова:
...
На вашем компьютере есть проект со внесенными изменениями, но вы хотите поделиться новой версией со всей командой. Перед "пушем" надо связать локальный и удаленный репозитории. Делается это при помощи команды:
...
### Перед "пушем" надо зафиксировать текущие изменения, то есть сделать *git commit -m*.
